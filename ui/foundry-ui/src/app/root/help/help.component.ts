...
export class HelpComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.$.push(this.customSettingService.load('helpPageText')
      .subscribe((result: SettingDetail) => {
        if (result) {
          if (result.value.length) {
            this.helpText = result.value;
          } else {
            // tslint:disable-next-line:max-line-length
            this.helpText = '####View the latest [CyberForce User Guide <i class="fa fa-download"></i>](./assets/docs/cyberforce-user-guide.pdf).';
          }
        } else {
          // tslint:disable-next-line:max-line-length
          this.helpText = '####View the latest [CyberForce User Guide <i class="fa fa-download"></i>](./assets/docs/cyberforce-user-guide.pdf).';
  ...
  renderedDescription() {
    return this.converter.makeHtml(this.helpText);
  }
