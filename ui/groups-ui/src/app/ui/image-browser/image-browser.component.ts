...
import { DataFilter, FileDetail } from '../../models';
import { ImagesService } from '../../svc/images.service';
...
export class ImageBrowserComponent implements OnInit {
  ...
  ngOnInit() {
   this.reset();
   this.imagePath = this.settingsSvc.settings.urls.uploadUrl;
  }
  ...
  search() {
    ...
    this.imgSvc.list(this.dataFilter).subscribe(result => {
  ...
  uploadFile(file: File) {
    this.imgSvc.uploadCrop(this.fileName, file).subscribe((result: any) => {
  ...
  showUrlButton() {
    if (this.brokenImage || !this.logoUrl) {
      return false;
    } else {
      return true;
    }
