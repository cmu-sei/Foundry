...
namespace Foundry.Groups.ValidationRules
{
    ...
    public class GroupNameIsUnique :
        IValidationRule<GroupCreate>,
        IValidationRule<GroupUpdate>
    {
        public GroupNameIsUnique(GroupsDbContext dbContext)
        {
            DbContext = dbContext;
        }
        ...
        public async Task Validate(GroupCreate model)
        {
            ...
            if (DbContext.Groups.Any(g => g.Name.ToLower() == model.Name.ToLower()))
                throw new InvalidModelException("A group with the name '" + model.Name + "' already exists");
        }
        ...
        public async Task Validate(GroupUpdate model)
        {
            ...
            if (DbContext.Groups.Any(g => g.Name.ToLower() == model.Name.ToLower() && g.Id != model.Id))
                throw new InvalidModelException("A group with the name '" + model.Name + "' already exists");
        }
    }
}
