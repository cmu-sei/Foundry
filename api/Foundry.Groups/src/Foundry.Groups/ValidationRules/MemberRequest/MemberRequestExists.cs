...
namespace Foundry.Groups.ValidationRules
{
    ...
    public class MemberRequestExists :
        IValidationRule<MemberRequestUpdate>,
        IValidationRule<MemberRequestDelete>
    {
        public MemberRequestExists(GroupsDbContext dbContext)
        {
            DbContext = dbContext;
        }
        ...
        public async Task Validate(MemberRequestUpdate model)
        {
            if (!DbContext.MemberRequests.Any(g => g.GroupId == model.GroupId && g.AccountId == model.AccountId))
                throw new EntityNotFoundException("Member request does not exist.");
        }
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public async Task Validate(MemberRequestDelete model)
        {
            if (!DbContext.MemberRequests.Any(g => g.GroupId == model.GroupId && g.AccountId == model.AccountId))
                throw new EntityNotFoundException("Member request does not exists");
        }
    }
}
