...
{
    public class MemberNotification : NotificationCreateStrategy
    {
        string _label;

        public MemberNotification(CommunicationOptions communicationOptions, IDomainEvent domainEvent)
            : base(communicationOptions, domainEvent) { }

        public override async Task<NotificationCreate> GetModel()
        {
            var model = DomainEvent.Entity as MemberNotificationModel;
