...
namespace Foundry.Groups.Tests
{
    public class TestIdentityResolver : IStackIdentityResolver
    {
        ...
        public TestIdentityResolver(Account account)
        {
            _account = account;
        }
        ...
        public async Task<IStackIdentity> GetIdentityAsync()
        {
            ...
            if (_account.IsAdministrator)
                permissions.Add("administrator");

            return new ProfileIdentity() { Id = _account.Id, Name = _account.Name, Permissions = permissions.ToArray() };
        }
    }
}
