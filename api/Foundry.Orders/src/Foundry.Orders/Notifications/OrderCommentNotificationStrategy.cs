...
{
    public class OrderCommentNotificationStrategy : NotificationCreateStrategy
    {
        public OrderCommentNotificationStrategy(CommunicationOptions communicationOptions, IDomainEvent domainEvent)
            : base(communicationOptions, domainEvent) { }

        public override string Label => "add";

        public override string Type => "ordercomment";
        ...
        public async override Task<NotificationCreate> GetModel()
        {
            ...
            var notification = new NotificationCreate
            {
                ...
                Subject = "Order Has A New Comment",
                Body = string.Format("Order '{0}' has a new comment.", DomainEvent.Name.PadLeft(5, '0')),
