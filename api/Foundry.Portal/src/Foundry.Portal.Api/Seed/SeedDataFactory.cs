...
namespace Foundry.Portal.Api
{
    public class SeedDataFactory
    {
        ...
        SketchDbContext SketchDbContext { get; }
        SeedOptions SeedOptions { get; }

        public SeedDataFactory(IHostingEnvironment hostingEnvironment, SketchDbContext sketchDbContext, SeedOptions seedOptions)
        {
            HostingEnvironment = hostingEnvironment ?? throw new ArgumentNullException(nameof(hostingEnvironment));
            SketchDbContext = sketchDbContext ?? throw new ArgumentNullException(nameof(sketchDbContext));
            ...
            try
            {
                ...
                if (result.Entities.Any())
                {
                    foreach (var entity in result.Entities)
                    {
                        ...
                        {
                            item.Entity = entity;
                            SketchDbContext.Set<TEntity>().Add(entity);
                            SketchDbContext.SaveChanges();
