...
{
    public class PlaylistGroupFollowNotification : NotificationStrategy
    {
        public override DomainEventType DomainEventType => DomainEventType.PlaylistGroupFollow;
        ...
        public override async Task<NotificationCreate> Build()
        {
            ...
            {
                GlobalId = GlobalId,
                Subject = "Playlist Group Follower Added",
                Body = string.Format("A group followed playlist '{0}'.", entity.Name),
                Values = ToValues(),
