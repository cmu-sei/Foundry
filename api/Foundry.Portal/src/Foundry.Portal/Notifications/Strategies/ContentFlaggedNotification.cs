...
{
    /// <summary>
    /// send a notification to all channel owners and power users
    /// </summary>
    public class ContentFlaggedNotification : NotificationStrategy
    {
        public override DomainEventType DomainEventType => DomainEventType.ContentFlagged;
        ...
        public override async Task<NotificationCreate> Build()
        {
            ...
            var ids = new List<string> { entity.CreatedBy };

            ids.AddRange(PowerUsers);

            var notification = new NotificationCreate
            {
                GlobalId = GlobalId,
                Subject = "Content Flagged",
                Body = string.Format("Content '{0}' was flagged.", entity.Name),
                Values = ToValues(),
