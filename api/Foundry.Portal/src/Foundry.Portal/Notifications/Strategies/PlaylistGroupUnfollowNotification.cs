...
{
    public class PlaylistGroupUnfollowNotification : NotificationStrategy
    {
        public override DomainEventType DomainEventType => DomainEventType.PlaylistGroupUnfollow;
        ...
        public override async Task<NotificationCreate> Build()
        {
            ...
            {
                GlobalId = GlobalId,
                Subject = "Playlist Follower Deleted",
                Body  = string.Format("A group stopped following playlist '{0}'.", entity.Name),
                Values = ToValues(),
