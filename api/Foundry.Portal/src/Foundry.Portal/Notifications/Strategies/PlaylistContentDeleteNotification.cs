...
{
    public class PlaylistContentDeleteNotification : NotificationStrategy
    {
        public override DomainEventType DomainEventType => DomainEventType.PlaylistContentDelete;
        ...
        public override async Task<NotificationCreate> Build()
        {
            ...
            {
                GlobalId = GlobalId,
                Subject = "Playlist Content Deleted",
                Body = string.Format("Content '{0}' was removed from playlist.", DomainEvent.Name),
                Values = ToValues(),
