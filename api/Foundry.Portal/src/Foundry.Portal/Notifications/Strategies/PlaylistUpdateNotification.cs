...
{
    public class PlaylistUpdateNotification : NotificationStrategy
    {
        public override DomainEventType DomainEventType => DomainEventType.PlaylistUpdate;
        ...
        public override async Task<NotificationCreate> Build()
        {
            ...
            var ids = DbContext.ProfileFollowers.Where(pf => pf.Playlist.GlobalId.ToLower() == GlobalId).Select(p => p.Profile.GlobalId);

            var notification = new NotificationCreate
            {
                GlobalId = GlobalId,
                Subject = "Playlist Updated",
                Body = string.Format("Playlist '{0}' was changed.", entity.Name),
                Values = ToValues(),
