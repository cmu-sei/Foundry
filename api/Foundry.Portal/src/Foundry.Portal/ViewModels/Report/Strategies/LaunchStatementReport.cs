...
{
    public class LaunchStatementReport : ReportModel<LaunchStatement>, IReportModel
    {
        public const string AnalyticsUrl = "api/statements/launched";

        public LaunchStatementReport(SketchDbContext dbContext, AuthorizationOptions authorizationOptions, AnalyticsOptions analyticsOptions)
        ...
        public override void Run(ReportDataFilter dataFilter)
        {
            ...
            var launchStatements = JsonConvert.DeserializeObject<List<LaunchStatement>>(result);

            var globalIds = launchStatements.Select(r => r.Description.ToLower());
            var contents = DbContext.Contents.Where(c => globalIds.Contains(c.GlobalId.ToLower())).ToList();

            foreach (var statement in launchStatements)
            {
            ...
            var filtered = ApplyDataFilter(dataFilter, launchStatements.AsQueryable());
        ...
        public override IQueryable<LaunchStatement> Filter(ReportDataFilter dataFilter, IQueryable<LaunchStatement> query)
        {
        ...
        public override IOrderedQueryable<LaunchStatement> Sort(ReportDataFilter dataFilter, IQueryable<LaunchStatement> query)
        {
        ...
        public override string Name { get { return "Content Launch Report"; } }
        public override string Description { get { return "Generates Content Launch Report."; } }
    }
}
