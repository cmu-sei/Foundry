...
namespace Foundry.Portal.Identity
{
    ...
    public class SketchIdentityResolver : HttpIdentityResolver
    {
        ...
        public SketchIdentityResolver(IHttpContextAccessor httpContextAccessor, SketchDbContext dbContext, IProfileCache profileCache, IClientCache clientCache, ILogger<SketchIdentityResolver> logger)
            : base(httpContextAccessor)
        {
            DbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
            ProfileCache = profileCache ?? throw new ArgumentNullException(nameof(profileCache));
            ClientCache = clientCache ?? throw new ArgumentNullException(nameof(clientCache));
        ...
        ISketchIdentityStrategy GetStrategy(ClaimsPrincipal claimsPrincipal)
        {
            ...
            if (string.IsNullOrWhiteSpace(subject))
                return new ClientIdentityStrategy(DbContext, claimsPrincipal, ClientCache);

            return new ProfileIdentityStrategy(DbContext, claimsPrincipal, ProfileCache);
        }
    }
}
