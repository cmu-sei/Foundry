...
{
    public class ContentTypeIsRequired : IValidationRule<ContentCreate>, IValidationRule<ContentUpdate>
    {
        public async Task Validate(ContentCreate model)
        {
            ...
            EnsureContentTypeIsSet(model.Type);
        }
        ...
        void EnsureContentTypeIsSet(ContentType contentType)
        {
            if (contentType == ContentType.NotSet)
                throw new InvalidModelException("Content type is required.");
        }
    }
}
